...
// inline useApi.ts sketched an idealized shape (App.pack_id, App.state as
// one of 8 stage names, GateReport.checks[].passed, AuditEntry.id/kind).
// The real, running API is different — confirmed by Task 2's SvelteKit
// treatment (see docs/treatments/ui-svelte.md), which verified it directly
// against src/api.rs / src/state.rs / src/gates.rs / src/audit.rs /
// src/packs.rs in the main repo:
...
// audit) since this treatment and the SvelteKit / Solid treatments
// implement the identical contract independently — Task 5 compares all
// three on it.
...
export function useApi() {
  ...
  // copy .env.example to .env and set NUXT_PUBLIC_DEV_TOKEN locally. Fail
  // loudly rather than silently proceeding without auth.
  const { public: { devToken } } = useRuntimeConfig()
  if (!devToken) {
    throw new Error('NUXT_PUBLIC_DEV_TOKEN is not set. Copy .env.example to .env and set it.')
  }
  ...
  return {
    ...
    audit: (id: string) =>
      request<{ events: AuditEntry[] }>(`/apps/${id}/audit`).then((r) => r.events)
  }
}
