...
|---|---|---|---|
| 1 | #4 agent-driver routing architecture (per investigation 0002 D1) | 4a router-in-driver · 4b pack-declared routing policy · 4c verified escalation ladder | harvested → [decision 0001](../decisions/0001-agent-routing.md), `claude/phase-0-integration` (verified: 29 tests, 39/39 sim + 47/47 staging-backed pressure checks); treatment branches superseded — lessons live in the decision record (ref deletion returns 403 under this environment's Git credentials; delete claude/issue-4-treatment-{a,b,c} from the GitHub UI at leisure) |
