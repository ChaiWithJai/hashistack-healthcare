...
// real, running API is different — independently verified twice already
// (Task 2, SvelteKit: web-treatments/svelte/src/lib/api.ts; Task 3, Nuxt:
// web-treatments/nuxt/app/composables/useApi.ts) by reading src/api.rs,
...
// audit) since Tasks 2-4 (SvelteKit, Nuxt, Solid) implement the identical
// contract independently and Task 5 compares all three treatments on it.
// This file is a direct, deliberate copy of Task 2's api.ts (import syntax
// only differs — both are ESM/Vite) so Task 5's comparison isolates
// framework differences rather than API-shape differences.
